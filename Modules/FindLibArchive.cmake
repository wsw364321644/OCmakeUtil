...
set(LibArchive_NAMES archive libarchive)
set(LibArchive_NAMES_DEBUG archived libarchived)

set(LibArchive_static_NAMES archive_static libarchive_static)
set(LibArchive_static_NAMES_DEBUG archive_staticd libarchive_staticd)
...
if(LibArchive_INCLUDE_DIR AND EXISTS "${LibArchive_INCLUDE_DIR}/archive.h")
	...
	set(_LibArchive_VERSION_REGEX
		"^#define[ \t]+ARCHIVE[_A-Z]+VERSION[_A-Z]*[ \t]+\"(libarchive +)?([0-9]+)\\.([0-9]+)\\.([0-9]+)[^\"]*\".*$"
	)
	file(STRINGS "${LibArchive_INCLUDE_DIR}/archive.h"
		_LibArchive_VERSION_STRING
		LIMIT_COUNT 1
		REGEX "${_LibArchive_VERSION_REGEX}"
	)
	if(_LibArchive_VERSION_STRING)
		string(REGEX
			REPLACE
			"${_LibArchive_VERSION_REGEX}"
			"\\2.\\3.\\4"
			LibArchive_VERSION
			"${_LibArchive_VERSION_STRING}"
		)
...
select_library_configurations(LibArchive_static)

if(LibArchive_LIBRARY OR LibArchive_static_LIBRARY)
	set(LibArchive_LIBRARY_FOUND TRUE)
endif()
include(FindPackageHandleStandardArgs)
find_package_handle_standard_args(
	LibArchive
	REQUIRED_VARS
	LibArchive_LIBRARY_FOUND
...
if(LibArchive_FOUND)
	if(NOT TARGET LibArchive::LibArchive)
		...
		set_target_properties(LibArchive::LibArchive
			PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${LibArchive_INCLUDE_DIR}"
		)
		if(LibArchive_LIBRARY_RELEASE)
			...
			set_target_properties(LibArchive::LibArchive
				PROPERTIES IMPORTED_LOCATION_RELEASE "${LibArchive_LIBRARY_RELEASE}"
			)
		...
		if(LibArchive_LIBRARY_DEBUG)
			...
			set_target_properties(LibArchive::LibArchive
				PROPERTIES IMPORTED_LOCATION_DEBUG "${LibArchive_LIBRARY_DEBUG}"
			)
		endif()

	endif()

	if(NOT TARGET LibArchive::LibArchive_static)
		add_library(LibArchive::LibArchive_static UNKNOWN IMPORTED)

		set_target_properties(LibArchive::LibArchive_static
			PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${LibArchive_INCLUDE_DIR}"
		)
		if(LibArchive_static_LIBRARY_RELEASE)
			set_property(
				TARGET LibArchive::LibArchive_static
				APPEND
				PROPERTY
					IMPORTED_CONFIGURATIONS
					RELEASE
			...
			set_target_properties(LibArchive::LibArchive_static
				PROPERTIES IMPORTED_LOCATION_RELEASE "${LibArchive_static_LIBRARY_RELEASE}"
			)

		endif()
		if(LibArchive_static_LIBRARY_DEBUG)
			set_property(
				TARGET LibArchive::LibArchive_static
				APPEND
				PROPERTY
			...
			set_target_properties(LibArchive::LibArchive_static
				PROPERTIES IMPORTED_LOCATION_DEBUG "${LibArchive_static_LIBRARY_DEBUG}"
			)
		endif()
	endif()
endif()
