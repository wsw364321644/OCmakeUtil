...
function(ImportProgram ProgramName)
	...
	cmake_parse_arguments(IMPORT_PROGRAM
		"${options}"
		"${oneValueArgs}"
		"${multiValueArgs}"
		${ARGN}
	...
	set(WORKING_DIRECTORY
		${IMPORT_PROJECT_EXTERNAL_DIR_CACHE}/${ProgramName_Lower}${WORKING_DIRECTORY_SUFFIX}
	)
	...
	if(ProgramName STREQUAL "NASM")
		...
		if(NOT ${ProgramName}_FOUND)
			...
			if(${ProgramName}_EXECUTABLE)
				message(STATUS
					"Before Import Find ${ProgramName} EXE :${${ProgramName}_EXECUTABLE}"
				)
			elseif(${ProgramName_Upper}_EXECUTABLE)
				message(STATUS
					"Before Import Find ${ProgramName} EXE :${${ProgramName_Upper}_EXECUTABLE}"
				)
			endif()
		endif()
	endif()
	...
	if(NOT Perl_FOUND)
		if(NOT IMPORT_PROGRAM_URL)
			...
			if(HOST_IS_64BIT)
				...
				set(IMPORT_PROGRAM_URL
					https://strawberryperl.com/download/5.32.1.1/strawberry-perl-5.32.1.1-32bit-portable.zip
				)
			endif()
		endif()
	...
	if(NOT CMAKE_ASM_NASM_COMPILER)
		if(NOT IMPORT_PROGRAM_URL)
			...
			if(HOST_IS_64BIT)
				...
				set(NASM_URL
					https://www.nasm.us/pub/nasm/releasebuilds/2.16.03/win32/nasm-2.16.03-win32.zip
				)
			endif()
		endif()
...
function(ImportPython3)
	set(WORKING_DIRECTORY
		${IMPORT_PROJECT_EXTERNAL_DIR_CACHE}/${ProgramName_Lower}
	)
	...
	if(NOT Python3_FOUND)
		if(NOT IMPORT_PROGRAM_URL)
			...
			if(HOST_IS_64BIT)
				set(IMPORT_PROGRAM_URL
					https://mirrors.aliyun.com/python-release/windows/python-3.13.5-embed-amd64.zip
				)
			else()
				set(IMPORT_PROGRAM_URL
					https://mirrors.aliyun.com/python-release/windows/python-3.13.4-embed-win32.zip
				)
			endif()
		endif()
		...
		if(PATH_FILE_LENGTH EQUAL 1)
			...
			message(FATAL_ERROR
				"Python _pth file not found or multiple found: ${PATH_FILE}"
			)
		endif()
	endif()
	...
	if(NOT EXISTS ${protoc_PATH}/bin/protoc.exe)
		if(NOT IMPORT_PROGRAM_URL)
			...
			if(HOST_IS_64BIT)
				...
				set(IMPORT_PROGRAM_URL
					https://github.com/protocolbuffers/protobuf/releases/download/v31.1/protoc-31.1-win32.zip
				)
			endif()
		endif()
...
function(ImportSteamLanguageParser)
	...
	if(NOT STEAMLANGUAGEPARSER_FOUND)
		FetchContent_Declare(
			download_SteamLanguageParser
			PREFIX
			${WORKING_DIRECTORY}
			GIT_REPOSITORY
			https://github.com/wsw364321644/SteamLanguageParser.git
			GIT_TAG
			main
			GIT_SHALLOW
			TRUE
			SOURCE_SUBDIR
			"avoid_add_subdirectory"
		)
		...
		execute_process(
			COMMAND ${CMAKE_COMMAND} ${CMAKE_GENERATOR_ARGV} -D
				"IMPORT_PROJECT_EXTERNAL_DIR=${IMPORT_PROJECT_EXTERNAL_DIR_CACHE}" -B build
				${download_steamlanguageparser_SOURCE_DIR}
		...
		file(RENAME
			${WORKING_DIRECTORY}/build/rundir/bin
			${SteamLanguageParser_NEW_PATH}
		)
